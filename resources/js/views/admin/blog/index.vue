...
export default {
  data() {
    return {
      ...
      headers: [
        ...
        { title: "Image", key: "image_preview", width: "100px", sortable: false },
        { title: "Title", value: "title" },
        { title: "Category", key: "category", sortable: false },
        { title: "Status", value: "status", width: "120px" },
        { title: "Date", value: "date", width: "130px" },
  ...
  methods: {
    ...
    async loadItems() {
      this.loading = true;
      try {
        const res = await General.get("admin/blog", this.filter);
        ...
        console.error(error);
        this.$alertStore?.add(error.message || "Failed to load blogs", "error");
      } finally {
        this.loading = false;
      }
    },
    ...
    async deleteItem(id) {
      ...
      this.loading = true;
      try {
        await General.delete(`admin/blog/${id}`);
        this.$alertStore?.add("Blog deleted successfully", "success");
        this.loadItems();
      } catch (error) {
